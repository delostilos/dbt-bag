...
def model(dbt, session):
    # tbv van lineage
    dbt.source('lz_bag','woonplaats_zipped_xml')
    dbt.source('lz_bag','woonplaats_inactief_zipped_xml')
    dbt.source('lz_bag','woonplaats_niet_bag_zipped_xml')
    # ophalen config
    ...
    print(f"{datetime.now(timezone.utc).strftime('%H:%M:%S')}  {area}{mnemonic}{datum}.zip uitgepakt")
    # in actief
    ZipFile(f"{sourcedir}{area}IA{mnemonic}{datum}.zip", 'r').extractall(tmpdirname)
    print(f"{datetime.now(timezone.utc).strftime('%H:%M:%S')}  {area}IA{mnemonic}{datum}.zip uitgepakt")        
    # niet BAG
    ZipFile(f"{sourcedir}{area}NB{mnemonic}{datum}.zip", 'r').extractall(tmpdirname)
    print(f"{datetime.now(timezone.utc).strftime('%H:%M:%S')}  {area}NB{mnemonic}{datum}.zip uitgepakt") 
